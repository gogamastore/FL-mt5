...
class RiskManager:
    ...
    def build_plan_levels(self, symbol: str, direction: str, sl: float,
                          tp: float, risk_percent: float | None = None) -> TradePlan | None:
        """Hitung lot dari level SL/TP yang sudah ditentukan (dipakai mode scalping)."""
        info = self.mt5.symbol_info(symbol)
        price = self.mt5.current_price(symbol)
        acc = self.mt5.account_info()
        if not info or not price or not acc:
        ...
        volume = max(info.volume_min, min(info.volume_max, round(volume / step) * step))
        digits = info.digits
        return TradePlan(volume=round(volume, 2), sl=round(sl, digits), tp=round(tp, digits))
